...
{\bf Input model.} Our input model is a $L=3$ layered GRU model, as defined in the deep learning framework PyTorch \textcolor{red}{Mettre un lien vers l'aide Torch ?}.
...
\begin{figure*}[htpb]
	\centering
	\includegraphics[width=.8\linewidth]{filter_kp1_hum.png}
	\includegraphics[width=.8\linewidth]{filter_kp1_co2.png}
...
\begin{figure*}[htpb]
	\centering
	\includegraphics[width=.8\linewidth]{filter_kp24_hum.png}
	\includegraphics[width=.8\linewidth]{filter_kp24_co2.png}
	\caption{Prediction of humidity (top) and CO2 (bottom) given observation ($t<120$) and without ($t > 120$). Since re sampling of particles is not longer available, the uncertainty grows. As a comparison, we plotted the predictions of the pretrained network, before implementing SMC methods.}%
	\label{fig:filter_k+24}
\end{figure*}

\begin{figure*}[htpb]
	\centering
	\includegraphics[width=.8\linewidth]{boxplot_comparison_hum.png}
	\includegraphics[width=.8\linewidth]{boxplot_comparison_co2.png}
...
\begin{table}[htpb]
	\centering
	\caption{Comparison of PI metrics of our model and the LSTM Dropout. For our model, we defined the lower and upper bound as the $q=0.95$ quantile of the predictions associated with each particle. Particles are propagated with access to the observations for 5 days, and evaluated on the two following days without. Mean values of the estimator on the validation set are displayed, along with their variance.}
	\label{tab:ci_comparison}
	\begin{tabular}{lll}
		\toprule
		                                    & PICP              & MPIW               \\
		\toprule
		Ours                                & $0.91 \pm 0.0204$ & $1.600 \pm 0.1263$ \\
		MC Dropout $p_{\texttt{drop}}=0.15$ & $0.32 \pm 0.0099$  & $0.383 \pm 0.0013$ \\
		MC Dropout $p_{\texttt{drop}}=0.3$  & $0.33 \pm 0.0144$ & $0.401 \pm 0.0013$ \\
		\bottomrule
	\end{tabular}
\end{table}
