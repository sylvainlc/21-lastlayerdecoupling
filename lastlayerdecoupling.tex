...
\begin{algorithm}
	...
	$\hat \theta \gets$ Gradient descent on $\widehat {S}^N_\theta(Y_{1:T})$.
\end{algorithm}
...
We also experimented with a Gaussian linear Hidden Markov Model (HMM), with a latent dimension of $4$, whose parameters are estimated with the Kalman smoother using the Expectation Maximization (EM) algorithm.
