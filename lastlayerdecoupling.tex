...
At each time $t$, a state $h_t$ is computed recursively: $h_t = g_\theta(h_{t-1},U_t)$ and $Y_t$ is predicted by a transform $f_\theta(h_t)$, where $\theta$ is a set of real-valued parameters of the networks (weights and bias) and $f_\theta$ and $g_\theta$ are parametric functions.
The parameters are trained using stochastic gradient based algorithms with a regularized mean squared error loss function.
...
The parameter $\varphi$ contains all unknown weights and biases associated with the input model.
