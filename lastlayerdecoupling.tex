...
Lastly, we denote by $\Psi_{\mu, \Sigma}$ the Gaussian probability density function with mean $\mu$ and covariance matrix $\Sigma$.
...
The conditional distribution of $X_{1:T}$ given $Y_{1:T}$ is not available explicitly for the nonlinear state space model, but it can be approximated using a family of particles $(\xi^{\ell}_{1:T})_{\ell=1}^N$ associated with importance weights $(\omega^{\ell}_T)_{\ell=1}^N$.
