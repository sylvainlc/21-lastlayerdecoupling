...
\begin{table}[htpb]
	\centering
	\caption{Comparison of PI metrics of our model and the LSTM Dropout. For our model, we defined the lower and upper bound as the $q=0.95$ quantile of the predictions associated with each particle. Particles are propagated with access to the observations for 5 days, and predictions are then sampled for the two following days. Two versions of the dropout model were evaluated, with dropout values $p_\textnormal{drop}=0.15$ and $p_\textnormal{drop}=0.3$. Mean values of the estimator on the validation set are displayed, along with their variance.}
