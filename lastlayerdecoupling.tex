...
\end{align}
where $\theta$ is a set of real-valued parameters of the networks (weights and biases) and $f_\theta$ and $g_\theta$ are non linear parametric functions.
...
\begin{equation}
	...
	\label{eq:grad_ll}
\end{equation}
where $\mathbb{E}_\theta$ design the expectation under the model of Section~\ref{sub:proposed_architecture} parameterized by $\widehat \varphi$ and $\theta$ (the dependency on the input $U_{1:T}$ is kept implicit here for better clarity). The conditional distribution of $X_{1:T}$ given $Y_{1:T}$ is not available explicitly for the nonlinear state space model proposed in this letter. %A solution to approximate \eqref{eq:grad_ll} is to obtain expectedly low variance Monte Carlo estimates of the smoothed expectation.
...
We chose a multi-layered GRU network, such that each hidden state is computed as follows, for all $1 \leq \ell \leq L$ and all $1 \leq t \leq T$, $ \tilde U^\ell_t = h_\varphi^\ell(\widetilde U^{\ell-1}_t, \widetilde U^\ell_{t-1})$, where the first layer is assimilated to the input vectors, $\widetilde U_t^0 \equiv U_t$, and $\widetilde U^\ell_0 \equiv 0$. The specific choice of functions $ h_\varphi^\ell$ are detailed in Section~\ref{sec:exp}.
...
% TODO input model loss function is MSE
...
\textbf{The Sequential Monte Carlo layer} uses the simple Path-space smoother. At $k = 0$, $(\xi^{\ell}_0)_{\ell=1}^N$ are sampled independently from the instrumental density $\rho_0$ and each particle $\xi^{\ell}_0$ is associated with the standard importance sampling weight: \textcolor{red}{quelle distribution initiale $\chi$ et quelle facon de simuler $\rho_0$}
\[
	\omega_0^{\ell} \propto \chi\left(\xi^{\ell}_0\right)\ell_0 \left(\xi^{\ell}_0\right)/ \rho_0 \left(\xi^{\ell}_0\right)\,,
\]
where $\ell_0(\cdot)$ is the Gaussian probability density function with mean $Y_0$ and variance $\Sigma_y$, evaluated at $f_\theta(\cdot)$. Then, for $k\geq 1$, using $\{(\xi^{\ell}_{k-1},\omega^{\ell}_{k-1})\}_{\ell=1}^N$, we sample pairs $\{(I^{\ell}_k,\xi^{\ell}_{k})\}_{\ell=1}^N$ of indices and particles from the instrumental distribution:
\[
	\pi_{k}(\ell,x) \propto \omega_{k-1}^{\ell} p_k(\xi^{\ell}_{k-1},x)\,.
\]
In this application we use for $p_k(\xi^{\ell}_{k-1},\cdot)$ the prior kernel, i.e. the Gaussian distribution with mean $g_{\theta}(\xi^{\ell}_{k-1},\tilde U_k)$ and variance $\Sigma_x$. For $\ell \in \{1,\ldots,N\}$, $\xi^{\ell}_k$ is associated with the following importance weight $\omega^{\ell}_k \propto \ell_k(\xi^{\ell}_k)$, where $\ell_k(\cdot)$ is the Gaussian probability density function with mean $Y_k$ and variance $\Sigma_y$, evaluated at $f_\theta(\cdot)$. We can approximate \eqref{eq:grad_ll} with:
$$
	\widehat {\nabla_\theta \log p_\theta(Y_{1:T})_k^N} = \sum_{i=1}^N \omega_T^i\nabla_\theta\log p_\theta(\xi^i_{1:T}, Y^{k}_{1:T})\,,
$$
where $\xi^i_{1:T}$ is the ancestral line of $\xi^i_{T}$. During forecasting, we implemented a bootstrap filter where particles are sampled from the prior model and weighted with the observation model.
